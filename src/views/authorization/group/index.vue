<template>
  <div class="person">
    ...
    <div class="table-wrapper">
      ...
      >
        ...
        <el-table-column
          ...
          <el-table-column prop="name" :label="t('授权')+t('名称')" />
        <el-table-column prop="personNumber" :label="t('授权')+t('人数')"  />
        <el-table-column prop="deviceNumber" :label="t('授权')+t('设备数')"  />
        <el-table-column prop="passRuleName" :label="t('通行规则')"  />

        <el-table-column fixed="right" :label="t('操作')" width="400">
          <template #default="{row}">
            <el-button  type="primary" size="small" @click="handle_travel(row)">{{t('通行周期')}}</el-button>
            <el-button  type="primary" size="small" @click="handle_device(row)">{{t('设备授权')}}</el-button>
            <el-button  type="primary" size="small" @click="handle_person(row)">{{t('人员授权')}}</el-button>
    ...
    <travel_drawer :groupID="group_id" :drawer_flag="travel_falg" @save="handle_travel_save"></travel_drawer>
 <person_drawer :drawer_person="person_drown" :groupID="group_id" @save_person="handle_save_person"></person_drawer>
    <device_drawer :groupID="group_id" @save_device="handle_save_device" :device_dia="device_dialog"></device_drawer>
  </div>
</template>
...
const group_list = ref({"current": 0,
    "pages": 0,
    "records": [
  {
    "deviceNumber": 0,
