...
import {device_GroupDevice} from "@/api/management";
...
const handleCurrentChange = async (val: number) => {
  ...
  await personnel_device_all(plotId, current_page.value, page_size.value, device_type.value, true, search_person.value)
};
//搜索
const handle_search = async () => {
  console.log('search_person.value', search_person.value)
  await personnel_device_all(plotId, 0, page_size.value, device_type.value, true, search_person.value)
  console.log('搜索')
}
const handle_close = () => {
  console.log('取消')
...
//获取所有数据
const personnel_device_all = async (plotId: number, page: any, size: any, deviceType: string, isExcludeAddedGroupDevice: boolean, serialNo?: string) => {
  const res = await device_GroupDevice(plotId, page, size, deviceType, isExcludeAddedGroupDevice, serialNo)
...
watch(() => props.dialog,
    (newVal) => {
      ...
      personnel_device_all(plotId, current_page.value, page_size.value, device_type.value, true)
