...
const handle_search =async ()=>{
  ...
  await personnel_list_all(tree_node.value, 0, page_size.value, 1,parentPath.value,search_person.value)
}
...
const handleCurrentChange = (val:number) => {
  ...
  select_all.value =false
   personnel_list_all(tree_node.value, val, page_size.value, 1,parentPath.value, search_person.value)
};
//点击table选择框
...
const handleNodeClick = (data: any) => {
  ...
  parentPath.value = data.parentPath
   personnel_list_all(tree_node.value, current_page.value, page_size.value, 1,parentPath.value)
...
const personnel_list_all = async (organizationId:any, page:number, size:number,status:number, parentPath:string, name?:string) => {
  const res = await personnel_groupList(organizationId, page, size, status, parentPath,true, name);
  if (res.data.code === 200) {
...
watch( () => props.dialog,
  async  (newVal) => {
      ...
      await personnel_list_all(tree_node.value, current_page.value, page_size.value, 1,parentPath.value)
