    ...
    <div class="right">
      ...
      <div v-else>
        <el-table
            ...
            style="width: 100%"
        >
          <el-table-column label="账户信息" prop="nickname"/>
          <el-table-column label="用户账号" prop="username"/>
          <el-table-column label="用户密码" prop="nickname"/>
          <el-table-column :label="t('操作')" fixed="right" width="200">
            <template #default="{row}">
              ...
              <el-button size="small" type="danger" @click="handle_Unbind(row)"> {{ t('解绑') }}</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
import {plot_move, reset, seach_list, user_list} from "@/api/system";
