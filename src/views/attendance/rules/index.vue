  ...
  <div class="rules">
    ...
    <div class="rules_table">
      <el-table
          ...
          style="width: 100%"
      >
        <el-table-column  prop="name" :label="t('规则名称')"  />
        <el-table-column prop="type" :label="t('考勤类型')" >
          <template #default="{row}">
            {{row.type===1?t('固定班制'):row.type===2?t('排版制'):t('自由班制')}}
          </template>
        </el-table-column>
        <el-table-column prop="isHolidayOff" :label="t('节假日排休')" >
          <template #default="{row}">
            {{row.isHolidayOff===true?t('是'):t('否')}}
          </template>
        </el-table-column>
        <el-table-column prop="isOvertimeStat" :label="t('加班统计')"  >
