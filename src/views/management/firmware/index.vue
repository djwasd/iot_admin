  ...
  <div class="person">
    ...
    <div class="table-wrapper">
      ...
      >
        ...
        <el-table-column prop="model" show-overflow-tooltip :label="t('设备')+t('型号')" />
        <el-table-column prop="version" show-overflow-tooltip :label="t('固件版本')" />
        <el-table-column prop="size"  show-overflow-tooltip :label="t('固件大小')" >
          <template #default="{row}">
            {{formattedSize(row)}}
          </template>
        </el-table-column>
        <el-table-column prop="uploadDatestamp"  show-overflow-tooltip :label="t('上传时间')" >
          <template #default="{row}">
            {{format_data(row.uploadDatestamp)}}
          </template>
        </el-table-column>
        <el-table-column prop="status"  show-overflow-tooltip :label="t('上传时间')" >
          <template #default="{row}">
            <el-switch
                v-model="row.status"
                class="ml-2"
