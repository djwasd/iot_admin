 ...
 * @param router
 */
function isProtectedPath(path:any) {
  const protectedPaths = [PageEnum.LOGIN, PageEnum.REGISTER, PageEnum.FORGET];
...
function createPermissionGuard(router: Router) {
  ...
  router.beforeEach(async (to) => {
    const protectedPaths = [PageEnum.LOGIN, PageEnum.REGISTER, PageEnum.FORGET];
    if (!isProtectedPath(to.path) && !userStore.token) {
      console.log('处理页面权限');
      await router.replace({ path: PageEnum.LOGIN, query: { redirect: to.fullPath } });
      return false;
    }

        // if (to.path !== PageEnum.LOGIN && !userStore.token && to.path !== PageEnum.REGISTER && to.path !== PageEnum.FORGET) {
    //   console.log('处理页面权限')
    //   await router.replace({ path: PageEnum.LOGIN, query: { redirect: to.fullPath } });
    //   return false;
    // }
