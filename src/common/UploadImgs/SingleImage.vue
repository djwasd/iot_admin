<template>
	<div class="upload-box">
		...
		>
            ...
            <mel-icon-Edit></mel-icon-Edit>
						<span>编辑</span>
					</div>
					<div class="handle-icon" @click="imgViewVisible = true">
            <mel-icon-ZoomIn></mel-icon-ZoomIn>
...
const { t } = useLocalesI18n({}, [(locale: string) => import(`../lang/${locale}.json`), 'common']);
import { generateUUID } from "@/utils/packagingmethod/utils";
...
const handleHttpUpload = async (options: UploadRequestOptions) => {
  ...
  const res = await  upload_attachments({
	...
	reader.onload = ()=>{
		r = reader.result
		emit("update:imageUrl", r,res.data.data);  //拿到上传图片的base64
	}
...
const beforeUpload: UploadProps["beforeUpload"] = rawFile => {
	...
	if (!imgSize)
		setTimeout(() => {
			ElNotification({
        title: t('温馨提示'),
				message:t('上传图片大小不能超过')+ ` ${props.fileSize}M！`,
				type: "warning"
			});
		}, 0);
